...
fn (mut g Gen) gen_fn_decl(node &ast.FnDecl, skip bool) {
	...
	*/
	...
	cur_closure_ctx := if is_closure { closure_ctx_struct(node) } else { '' }
	if is_closure {
...
fn (mut g Gen) gen_anon_fn(mut node ast.AnonFn) {
	...
	g.empty_line = false
	g.writeln('__closure_create($node.decl.name, ${node.decl.params.len + 1}, &(${closure_ctx_struct(node.decl)}) { ')
	g.indent++
